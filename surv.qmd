...
```{r fun-example,h=3.5,w=4.25,cap='Survival function'}
...
plot(tt, survival, type="l", xlab="t", ylab="Survival Function")
```


```{r cumhaz-example,h=3.5,w=4.25,cap='Cumulative hazard function'}
#| label: fig-surv-cumhaz-example
spar(ps=12,bty='l')
plot(tt, cumhaz, type="l", xlab="t",
...
```{r hazard-example,h=3.5,w=4.25,cap='Hazard function'}
#| label: fig-surv-hazard-example
spar(ps=12,bty='l')
plot(tt, hazard, type="l", xlab="t", ylab="Hazard Function")
```
...
```{r weibull-shapes,cap='Some Weibull hazard functions with $\\alpha=1$ and various values of $\\gamma$'}
#| label: fig-surv-weibull-shapes
spar(bty='l')
tt <- seq(1e-5, 1.2, length=100)
plot(tt, rep(0,100), ylim=c(0,7), type="n", xlab="t", ylab="")
a <- 1
i <- 0
for(b in c(.5,1,2,4)) {
	i <- i + 1
