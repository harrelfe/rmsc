...
```{r lookdist,w=5.5,h=5.5,cap='Examination of normality and constant variance assumption, and assumptions for various ordinal models',scap='Examining normality and ordinal model assumptions'}
...
* Upper right curves are not linear, implying that a normal `r ipacue()`
...
: Distribution families used in ordinal cumulative probability models. $\Phi$
  denotes the Gaussian cumulative distribution function.  For the
  Connection column, $P_{1}=\Pr[Y \geq y | X_{1}], P_{2}=\Pr[Y
    \geq y | X_{2}], \Delta=(X_{2}-X_{1})\beta$.  The connection
  specifies the only distributional assumption if the model is
  fitted semiparametrically, i.e, contains
  an intercept for every unique $Y$ value less one.  For parametric
  models, $P_{1}$ must be specified absolutely instead of just
  requiring a relationship between $P_{1}$ and $P_{2}$.  For example,
  the traditional Gaussian parametric model specifies that
  $\Pr[Y \geq y | X] = 1 - \Phi(\frac{y - X\beta}{\sigma})
  = \Phi(\frac{-y + X\beta}{\sigma})$. {#tbl-cony-ormdist}
...
plot(z, xlim=c(4, 7.5), data=w[j,c('pghg', 'gh')]) 
```
...
```{r allocadf,h=3.5, w=4,cap='Generalized squared rank correlations'}
...
of the frequently-missing predictors is important.

```{r fitfullcasewise,results='asis'}
...
Next fit the reduced model.  Use multiple imputation to impute missing
 `r ipacue()`  predictors.
Do an ANOVA for the reduced model,  `r ipacue()`  taking imputation into account.


```{r sanova,results='asis'}
h <- function() {
