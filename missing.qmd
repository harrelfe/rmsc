...
`r quoteit("Consider a study of the effectiveness of a cardiac rehabilitation program.  Suppose a sample size of N=300 will result in an acceptable margin of error in estimating the overall treatment effect.  The investigators allow for 20% dropout so multiplies N by 1/0.8 = 1.25.  What serious mistake are they likely making?")`
...
* Predicted values need only be monotonically related to real
   ...
   $d_{i} = |\hat{y_{i}} - y^{*}|$ <br>
   $s = 0.2\times\text{mean} |\hat{y_{i}} - y^{*}|$ is a good default scale factor. <br>
...
* `fit.mult.impute` computes very approximate Wald statistics by multiplying the variance-covariance matrix computed on the stacked data by $M$ if individual completed dataset analyses are not done
...
* MCAR can be partially assessed by comparing distribution of `r ipacue()`
  ...
  subjects having incomplete $X$ [@lit02sta]
...
<img src="images/missing-diagnostic.png" width="60%">
