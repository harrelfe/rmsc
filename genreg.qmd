...
* Isolate effects of variables (especially exposure and treatment)
* Hypothesis testing
...
::: {.column-screen-left}
<img src="images/kap23lea.jpg" width="100%">
:::
...
[This paper](http://yiqingxu.org/papers/english/2018_HMX_interaction/main.pdf) describes how interaction effects can be misleading.  See [Interaction effects need interaction controls](https://datacolada.org/80) by Uri Simonsohn for an explanation of the possible need for controlling for other interactions when the interest is on a specific interaction.  <!-- NEW -->
...
* Don't forget what the test is powered to detect
...
association.  If an interaction effect is present, there is
...
<img src="images/lucy.png" width="45%"> &nbsp;&nbsp;&nbsp;  Lucy D'Agostino McGowan
...
* Categorizing continuous predictors into intervals is a
  ...
  @bel92con, @far96sim, @rag92dic, @sui95bin, @bue97pro, 
  @max93biv, @sch94out, @alt98sub, @hol04con, @mos04odd,
  @wai06fin, @fed09con, @gia14opt, @col16qua, @ben12qua 
...
1. Categorization only appears to yield interpretable estimates.
...
[Interactive demonstration](https://apps.biostat.au.dk/stefan/splines/) of lack of fit after categorization of a continuous predictor, and comparison with spline fits, by Stefan Hansen
...
<img src="images/nhanesIIIretinopathy.png" width="90%">
...
`r mrg(sound("genreg-5"))`
 `r ipacue()` 
@sto85add: cubic splines poorly behaved in tails.
Constrain function to be linear in tails. <br>
$k+3 \rightarrow k-1$ parameters @dev86spl.
...
[pclambert.net/interactive_graphs](https://pclambert.net/interactive_graphs.html).
Jordan Gauthier
...
<img src="images/sel10gly.png" width="100%">
...
* Small $n$ ($<30$, say) -- $k=3$
* Can use Akaike's information criterion (AIC) (@atk80not, @hou90pre) to choose $k$
...
* Moving linear regression far superior to moving avg. (moving
  flat line) `r ipacue()` 
* @cle79rob moving linear regression smoother _loess_ (locally weighted least squares) is the most popular
...
Regression splines have several advantages (@har88reg):
...
* Breiman's nonnegative garrote (@xio10not)
...
* sparse principal components analysis to achieve
  parsimony in data reduction @wit08tes,@zho06spa,@len09gen,@lee10spa
...
* See [A brief tour of deep learning from a statistical perspective](https://www.annualreviews.org/content/journals/10.1146/annurev-statistics-032921-013738) by E Nalisnick, P Smyth, D Tran, 2023.
...
@gra91eff elegantly described the hazards of
pretesting
...
* Parametric models make an absolute distributional assumption
   + E.g. linear regression: $Y \sim X\beta + \epsilon, \epsilon \sim n(0,\sigma^{2})$
   + Examine distribution of residuals
* Some models (Weibull, @cox72reg): <br>
    $C(Y|X) = C(Y=y|X) = d(y) + X\beta$ <br>
    $C =$ log hazard
* Check form of $d(y)$ for parametric models such as Weibull
* For semiparametric models (Cox and ordinal regression) $d(y)$ does not need to be specified
   + $d(y)$ is estimated by a step function (is link function of empirical CDF when $\beta=0$)
* Show $d(y)$ does not interact with $X$ (parallelism assumption)
...
$$
\begin{aligned}
E(Y | x, z) = f(x, z) &= \beta_0 + \beta_1 x + \beta_2 x^2 + \beta_3 x^3 \\
                      &+ \beta_4 [x > 3] + \beta_5 z
\end{aligned}
$$
...
<!--
# Put single d.f. contrast in first column
# For each nonzero element of D add the original X column divided by that D element
xc <- rep(0, n)
for(i in 1 : length(D))
  if(D[i] != 0) xc <- xc + X[, i] / D[i]
# Divide by number of non-zero D's so that when adding all these xc components we get 0-1 scale for 
...
* obtaining exact inference about unions and intersections of various assertions about parameters, e.g., the probability that a treatment reduces mortality by any amount or reduces blood pressure by $\geq5$ mmHg
...
### Help in Specifying Priors {#sec-genreg-prior}
...
1. Analysts want to use least squares multiple regression to
  predict follow-up blood pressure from baseline blood pressure, age,
  and sex.  They want to use this model for estimation, prediction,
  and inference (statistical tests and confidence limits).  List all
  of the assumptions they make by fitting the model $Y = \beta_{0} +
