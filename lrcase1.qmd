...
# transformed ones
pc8 <- princmp(~ sz + sg + log(ap) + sbp + dbp + age +
           wt + hg + ekg.norm + pfn + bm + hx + rxn + dtime,
           data=psub, k=8, sw=TRUE, kapprox=2, orig=FALSE)
f8   <- lrm(cvd ~ pc8, data=psub)
pc8t <- princmp(trans, k=8, sw=TRUE, kapprox=2, orig=FALSE)
...
```{r h=3.5,w=3.5,cap='Ranking of apparent importance of predictors of cause of death'}
