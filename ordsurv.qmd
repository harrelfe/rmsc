...
```{mermaid}
flowchart LR
PM[Parametric Models]
SP[Semiparametric Models]
PM --> PMA[Assumptions<br><br>Link-transfored Y distributions parallel<br><br>Y perfectly transformed<br><br>Y has a specific distribution<br><br>Usual X assumptions]
SP --> SPA[Assumptions<br><br>Link-transformed Y distributions parallel<br><br>Usual X assumptions]
```

```{mermaid}
flowchart LR
Y[Types of Y Allowed in<br>Semiparametric Models]
Y --> binary
Y --> d[discrete ordinal]
Y --> fc[floor and ceiling effects<br>zero inflation]
Y --> disc[general discontinuities]
Y --> bim[bimodal distributions]
Y --> continuous
Y --> cens[left, right, interval censored]
```
...
* The Cox PH model is a special case of an ordinal model with a log-log link.  @cox72 developed a partial likelihood approach so make estimation of regression coefficients very easy and fast.  This approach required the intercepts (underlying survival curve) to be estimated in a second step once $\hat{\beta}$ were computed.  There is a degree of arbitrariness in this approach, and methods for computing confidence intervals for estimated survival probabilities are complex.  Ordinal models use full likelihood so they estimate the intercepts simultaneously with $\beta$.  This is about as fast as just estimating $\beta$ with Cox PH, because with right censoring the hessian matrix for ordinal models is extremely sparse, so inverting its negative in order to get the covariance matrix is extremely fast even with hundreds of thousands of distinct failure times.
...
* `Mean`: generate an R function that computes restricted mean survival time
* `plotIntercepts`: plot intercepts vs. corresponding $y$-values, with optional log scale for $y$; linearity of such plots would indicate that the variable would have been correctly transformed for the corresponding parametric model
...
The `Hazard` function generator is not available for ordinal models.  Discrete hazard functions are somewhat easy to compute using the `Survival` and `predict` functions.
...
* $\rightarrow$ no distributional assumption to check
...
* Embed the model in a more general model with more parameters
   ...
   + this is the preferred method but is not covered in this chapter although is related to the $y$-varying $\beta$ plots to follow
...
* The same as the previous dichotomization approach but on the overall linear predictor, seeing how it slope varies away from 1.0 over cut-points
...
So in non-huge datasets **some GOF assessments are noisy are hard to use for decision making**.
...
Another way: plot the proportion of times that the logit deviance was the smallest of the four, and the proportion of times that either the logit or probit were the smallest.
...
flowchart LR
S[Strategy]
S --> PS[Pre-specification]
S --> CF[Controlled flexibility]
PS --> LP[Choose link that<br>well-fitted<br>a similar dataset]
PS --> SM[Subject matter knowledge<br><br>Chronic process, constant<br>X effects over time:<br>PH model log-log link<br><br>Acute process, front-loaded X effects<br>accelerated failure time<br>logit link]
CF --> DEV[Compare deviances of<br>logit, log-log, c-loglog links]
CF --> CUT[Check flatness of overall<br>linear predictor effect<br>over y cutpoints]
CF --> SX[Predictor of special<br>importance<br><br>Check flatness of its<br>partial effect over<br>cutpoints]
```
...
Let's fit a series of models examine fitted linear predictors from the log-log and logit model.  `mscore=TRUE` computes the $n\times p$ score matrix correctly if censoring is present.
...
Look at internal calibration-in-the-small curves and also print statistics for calibration-in-the-large.  Cut-points are chosen so that there are 100 uncensored observations between cut-points and beyond outer cut-points.
...
```{r}
intCalibration(g11, m=100, x=age, dec=2) + labs(subtitle=g11$family)
intCalibration(g12, m=100, x=age, dec=2) + labs(subtitle=g12$fam)
```
...
```{r}
intCalibration(g21, m=100, x=age, dec=2) + labs(subtitle=g21$family)
intCalibration(g22, m=100, x=age, dec=2) + labs(subtitle=g22$family)
```
...
Derive R functions for translating the linear predictor into estimated mean and quantiles using the logic link.  For ordinal models fitted to right-censored data, the mean is the restricted mean survival time.  Until the `Mean.orm` function adds a `tmax` argument, the restriction is essentially to the lowest censored time that is above the highest uncensored time, which is `r minc`.
...
```{r plot, h=7, w=7, cap='Effect of each predictor on the normal inverse survival probability scale.  Predicted values have been centered so that predictions at predictor reference values are zero.  Pointwise 0.95 confidence bands are also shown.  As all $Y$-axes have the same scale, it is easy to see which predictors are strongest.',scap='Effect of predictors on log survival time in SUPPORT'}
#| label: fig-ordsurv-plot
ggplot(Predict(f, ref.zero=TRUE), vnames='names', anova=a)
```

```{r plot2, h=7, w=7, cap='Effect of each predictor on the restricted mean survival time scale.  Pointwise 0.95 confidence bands are also shown.',scap='Effect of predictors on RMST in SUPPORT'}
#| label: fig-ordsurv-plot2
ggplot(Predict(f, ref.zero=FALSE, fun=expected.surv), vnames='names')
```
...
```{r scurves, h=7, w=7, cap='Effect of each predictor on the normal inverse survival probability scale.  Predicted values have been centered so that predictions at predictor reference values are zero.  Pointwise 0.95 confidence bands are also shown.',scap='Probit AFT survival curves estimates for SUPPORT'}
