...
* $p= $ number of parameters in full model or number of _candidate_
...
* Univariable screening (graphs, crosstabs, etc.) {\bf in no way}
...
```{r shrink-groupmeans,ps=7,cap='Sorted means from 20 samples of size 50 from a uniform $[0,1]$ distribution.  The reference line at 0.5 depicts the true population value of all of the means.',scap='Means from 20 $U(0,1)$ samples'}
...
* OLS^[An excellent discussion about such indexes may be found [here](http://r.789695.n4.nabble.com/Adjusted-R-squared-formula-in-lm-td4656857.html).]:
...
```{r physiol-transcan,cap='Transformations fitted using `transcan`.  Tick marks indicate the two imputed values for blood pressure.',scap='`transcan` transformations for two physiologic variables'}
