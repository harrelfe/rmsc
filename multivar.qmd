...
* Rigorous judgements about information quality and sufficiency for explicit objectives should precede any modeling
   + Undertaking modeling and inference and subsequently apologizing for data deficiencies results in scientific noise
...
* See [On the uses and abuses of regression models](https://onlinelibrary.wiley.com/doi/10.1002/sim.10244) by JB Carlin and M Moreno-Betancur.  See also [this](https://bsky.app/profile/georgheinze.bsky.social/post/3lsj4jzbghs2a).
...
* If wish to preserve statistical properties, can't retrieve d.f.
  once they are "spent" (see @gra91eff)
...
* $F$ and $\chi^2$ statistics do not have the claimed
        distribution^[@loc13sig provide an example with $n=100$ and 10 orthogonal predictors where all true $\beta$s are zero. The test statistic for the first variable to enter has type I assertion probability of 0.39 when the nominal $\alpha$ is set to 0.05.] [@gra91eff]
...
Variable selection methods [@logist]:
`r mrg(sound("mv-5"))`
...
* Test for significance or use Akaike's information criterion
        (AIC [@atk80not]), here $\chi^{2}-2 \times d.f.$
...
* Full model fits gives meaningful confidence intervals with
        ...
        not [@alt89boo; @hur90imp; @bre92lit]
...
* [Best subset, forward stepwise or lasso?  Analysis and recommendations based on extensive simulations](https://www.stat.cmu.edu/~ryantibs/papers/bestsubset-sts.pdf) by Trevor Hastie, Robert Tibshirani, and Ryan Tibshirani, 2020
* [Variable selection results in worse predictions than general shrinkage such as ridge regression](https://stats.stackexchange.com/a/669294/4253)
...
variable selection.
 ...
 <img src="images/maxwellsdemon.jpg" width="65%"> <img src="images/maxwell.jpg" width="25%">[James Clerk Maxwell]{.aside}
...
* $p$ should be $< \frac{m}{15}$ [@har84reg;  @har85reg; @smi92pro; @ped95imp; @ped96sim; @vit06rel; @plo14mod]
* $p$ is number of parameters in full model or number of _candidate_
    parameters in a stepwise analysis, and $m$ is the effective sample size
   + See @sec-ordsurv for how to estimate the effective sample size coming from censored observations
...
| Binary                    | $3nq(1-q), q=\frac{n_{2}}{n}$ `r fa`  |
...
* Narrowly distributed predictor $\rightarrow$ even higher $n$ `r ipacue()`[See [this](https://stats.stackexchange.com/questions/655891) and [this](https://hbiostat.org/R/glossary.html) for more information.]{.aside}
...
* @spi86pro: var. selection arbitrary, better
...
* Does not harm predictions on model construction
  sample  `r ipacue()` 
* Does not affect predictions on new data [@mye90cla, pp. 379-381] if
...
* See [this](https://stats.stackexchange.com/questions/5774) [this](https://stats.stackexchange.com/questions/547566) and [this](https://stats.stackexchange.com/questions/3810) for useful resources related to principal component analysis of mixed continuous and categorical variables
* [Multivariate analysis of mixed data: The R package PCAmixdata](https://arxiv.org/abs/1411.4911) by M Chavent et al, 2022.
* R [homals package](https://cran.r-project.org/web/packages/homals)
...
* Use flexible additive parametric models to
...
1. Uses nonparametric "super smoother" [@fri84var] `r ipacue()`
...
* Leverage: capacity to be influential (not necessarily infl.) <br> `r ipacue()`
    ...
    an obs. predicts its own response [@bel80reg]
...
* Some classify obs as overly influential when
    $|{\rm DFFITS}|>2\sqrt{(p+1)/(n-p-1)}$ [@bel80reg]
* Others examine entire distribution for "outliers"
* No substitute for careful examination of data [@cha91avo; @spe93rem]
...
For an excellent tutorial covering many strategical aspects of developing and validating predictive models see @eft24dev.
...
1. Characterize subjects with missing $Y$.  Delete such subjects
        only in rare circumstances [@cra95com].  For certain models it
...
1. Use the entire sample in model development unless $n$ is very large
...
1. Do limited backwards step-down variable selection if parsimony
        is more important that accuracy [@spi86pro].  But confidence
...
1. Investigators are interested in finding risk factors for stroke.
  They collected data on 3000 subjects of whom 70 were known to
  suffer a stroke within 5 years from study entry.  They decided
  that race (5 possible levels), age, height, and mean arterial blood
  pressure should definitely be in the model.  They do not try to
