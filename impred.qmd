...
prin.raw <- princmp(~ sz + sg + ap + sbp + dbp + age +
          wt + hg + ekg + pfn + bm + hx,
          k=16, sw=TRUE, ylim=c(0, 4), offset=1, data=imputed)

prin.trans <- princmp(ptrans$transformed, k=12, add=TRUE, col='red', offset=-1.3)
```
...
pco <- function(v) {
  f <- princmp(ptrans$transformed[,v], pl=FALSE, scoef=FALSE, orig=FALSE)
  vars <- attr(f, 'vars')
