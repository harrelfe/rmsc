...
: Degrees of fredom needed for predictors {#tbl-impred-maxdf}
...
on raw candidate predictors, expanding polytomous factors using dummy variables.
The `R` function `princomp` is used, after singly imputing missing raw
...
The resulting plot shown in  @fig-impred-pc is called a
...
It requires at least 9 of the 12 possible components to explain $\geq 0.9$
...
@pcaPP have written a nice
`R` package `pcaPP` for doing sparse PC
analysis.^[The `spca` package is a new sparse PC package that should also be considered.]
